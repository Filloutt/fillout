...
const bad=template();bad[2].ins=['n3','n2'];assert.throws(()=>compile(bad),/Cycle/);
const missing=template();missing[2].ins[0]=null;assert.throws(()=>compile(missing),/Connect/);
assert.throws(()=>validateDraft({version:1,nodes:[{id:'<script>',type:'input'}]}),/node/);
